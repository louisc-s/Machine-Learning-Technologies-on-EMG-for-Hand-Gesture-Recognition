...
for subject in subjects:
    ...
    #retrieve input sEMG data and target labels from CSV files
    ...
    #expand dimensions of data inputs so they are suitable for 
    #input into 2D convolutional layer 
    inputs = np.expand_dims(inputs, axis=3)
    val_inputs = np.expand_dims(val_inputs, axis=3)
    ...
    model.add(Conv2D(16, kernel_size=5, strides=(1,1),padding= 'same', use_bias=False,input_shape=(40,10,1)))
    model.add(BatchNormalization())
    model.add(Activation(activations.relu))
    model.add(MaxPooling2D(pool_size=(2,2),strides=(1,1), padding='same'))

    #2nd convolutional layer
    model.add(Conv2D(32, kernel_size=5, strides=(1,1),padding= 'same', use_bias=False))
    model.add(BatchNormalization())
    model.add(Activation(activations.relu))
    model.add(MaxPooling2D(pool_size=(2,2),strides=(1,1), padding='same'))
    ...
    #test 2D CNN again using model.predict function so that performance metrics can be generated 
